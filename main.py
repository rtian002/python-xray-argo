...
UUID = os.environ.get('UUID', 'b0cc09a7-06f0-42fb-a161-17434162ebd9')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
SUB_PATH = os.environ.get('SUB_PATH', 'getmysubs')                  # 订阅token,默认sub，例如：https://www.domain.com/sub
NAME = os.environ.get('NAME', 'DB-rtian001')                        # 节点名称
...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'db1.auth.xx.kg')   # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiYWZhYmM2MWRlYTliMDU1Y2JjZjc3MThmNDA3YzRlYWMiLCJ0IjoiNzU3OTZjMGQtODkyOS00Y2IzLWI5MDItZDUyOGYyYmQzNDdmIiwicyI6Ik9ESTNNREUzWm1RdE5qSXpaQzAwT1RBMkxXRmtaV1V0WkdRMll6RTFZVEUwWmprMyJ9')            # Argo固定隧道密钥,留空即使用临时隧道
    ...
    def do_GET(self):
        if self.path == '/':
            ...
            if os.path.exists(os.path.join(filepath, 'index.html')):
                ...
                self.end_headers()
                with open(os.path.join(filepath, 'index.html'), 'rb') as f:
